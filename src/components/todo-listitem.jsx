    ...
    return (
        <div className='flex items-center'>
            <div className='flex-1 flex items-center gap-2 my-3 hover:cursor-pointer'>
                <img onClick={() => completeTask(id)} src={completed ? tick_icon : not_tick} className='w-6 h-6' alt="tick_icon" />
                <div className={'text-md text-slate-700 px-2 decoration-slate-500 ' + (completed ? 'line-through' : '') }>{text}</div>
            </div>
